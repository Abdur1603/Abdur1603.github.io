...
if (isset($_POST["submit"])) {
  ...
  if ($result->num_rows === 1) {
    ...
    if (password_verify($password, $user['password'])) {
      ...
      $_SESSION['login'] = true; // Menandai pengguna telah login
      $_SESSION['username'] = $user['username']; // Menyimpan username ke session
  
      // Cek role pengguna dan simpan ke session
      if ($user['role'] === 'Admin') {
        $_SESSION['role'] = 'admin'; // Set role admin di session
        echo "
        <script>
        ...
        $_SESSION['role'] = 'user'; // Set role user di session
        echo "
        <script>
